<?php
    ...
    $fno = $phone = $tdate = "" ;
    $fno_err = $phone_err = $tdate_err = "" ;
    $naam = $_SESSION["username"];    
    // Processing form data when form is submitted
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        // Check if flight no. is empty
        if(empty(trim($_POST["fno"])))
        { $fno_err = "Please enter plane number." ; } 
        else
        { $fno = trim($_POST["fno"]) ; }
        ...
        if(empty($fno_err) && empty($phone_err) && empty($tdate_err))
        {
            // Prepare an insert statement
            $sql = "INSERT INTO FlightInfo (username, phone, fno, tdate) VALUES (?, ?, ?, ?)";
            if($stmt = mysqli_prepare($link, $sql))
            {
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt, "ssss", $param_naam, $param_phone, $param_fno, $param_tdate);
                // Set parameters
                $param_naam = $naam;
                $param_phone = $phone;
                $param_fno = $fno;
...
<!-- Header -->
    <div class="starter">
        ...
        <p> This helps you to find people travelling in the same flight as yours. </p>
    </div>        
<!-- FORM -->            
    <div class="form">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <p class="sue-storm">  .   </p>
            <div class="form-group <?php echo (!empty($fno_err)) ? 'has-error' : ''; ?>">
                <i>FLIGHT NO : </i><br>
                <input type="text" name="fno" class="form-control" value="<?php echo $fno; ?>">
                <span class="help-block"><?php echo $fno_err; ?></span>
            </div>
            ...
            <div class="form-group">
                ...
                <p>Look for previous flight matches ?? <a href="ftable.php"> Click Here !! </a></p>
            </div>
        </form>
        <p class="sue-storm">  .   </p>
    </div>
